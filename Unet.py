...
def sinus_time_emb(ts,t_emb_dim):
    factor = 10000 ** ((torch.arange(
        start=0, end=t_emb_dim//2,device=ts.device,) / (t_emb_dim//2)
                        ))
    t_emb = ts[:,None].repeat(1,t_emb_dim//2) / factor
...
class DownBlock(nn.Module):
    def __init__(self,in_channels,out_channels,t_emb_dim,down_sample,num_heads):
        ...
        self.resnet_first_block = nn.Sequential(
        ...
        self.resnet_second_block = nn.Sequential(
    ...
    def forward(self,x,t_emb):
        ...
        ## Resnet Block 
        resnet_input = x
        out = self.resnet_first_block(out)
        out = out + self.t_emb_layer(t_emb)[:,:,None,None]
        out = self.resnet_second_block(out)
...
class MidBlock(nn.Module):
    def __init__(self, in_channels, out_channels, t_emb_dim, num_heads):
        ...
        self.resent_first_block = nn.ModuleList([
            nn.Sequential(
                nn.GroupNorm(8,in_channels),
                nn.SiLU(),
                nn.Conv2d(in_channels , out_channels , kernel_size=3,stride=1,padding=1)
        ...
        self.resent_second_block = nn.ModuleList([
            nn.Sequential(
                nn.GroupNorm(8,out_channels),
                nn.SiLU(),
                nn.Conv2d(out_channels , out_channels , kernel_size=3,stride=1,padding=1)
    ...
    def forward(self,x,t_emb_dim):
        ...
        resnet_input = x 
        ## First Resent Block 
        out = self.resent_first_block[0](out)
        out = out + self.t_emb_layer[0](t_emb_dim)[:,:,None,None]
        out = self.resent_second_block[0](out)
        ...
        ## Second ResNet Block 
        resnet_input = out
        out = self.resent_first_block[1](out)
        out = out + self.t_emb_layer[1](t_emb_dim)[:,:,None,None]
        out = self.resent_second_block[1](out)
        #print(f"test test test {resnet_input.shape}")
...
class UpBlock(nn.Module): 
    def __init__(self,in_channels , out_channels , t_emb_dim , up_sample,num_heads):
        ...
        self.residual_input_conv = nn.Conv2d(in_channels,out_channels,kernel_size=1)
        self.up_sample_layer = nn.ConvTranspose2d(in_channels//2,in_channels//2,
                                                  kernel_size=4,stride=2,padding=1) if up_sample else nn.Identity()
        
    
    def forward(self,x,out_down,t_emb_dim) : 
        ...
        ## Resnet Block 
        out = self.resnet_first_block(out)
        out = out + self.t_emb_layer(t_emb_dim)[:,:,None,None]
        out = self.resnet_second_block(out)
        out = out + self.residual_input_conv(resnet_input)
...
class Unet(nn.Module):
    ...
    def forward(self,x,t):
        out = self.conv_in(x)
        t_emb = sinus_time_emb(t,self.t_emb_dim)
        t_emb = self.t_proj(t_emb)
        
        down_outs =[]
        i = 0 
        for down in self.downs : 
            #print(f"Down samling : {out.shape}")
            down_outs.append(out)
            out = down(out,t_emb)
        for mid in self.mids : 
            i+=1
            #print("testing testing testing " , i)
            #print(f"Mid Block : {out.shape}")
            out = mid(out,t_emb)
            
        for up in self.ups : 
            down_out = down_outs.pop()
            #print(f"Up sampling : {down_out.shape}")
